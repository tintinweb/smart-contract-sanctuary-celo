...
class CeloExplorerApi(EtherScanIoApi):

    @retry(Exception, delay=1, backoff=2, max_delay=10, tries=5, jitter=(1,4), logger=logger)
    def _request_contract_list(self, page, amount=500):
        resp = self.session.get("/verified-contracts?order_dir=desc&order_field=date&page_number={page}&page_size={amount}".format(page=page, amount=amount))
        root = html.fromstring(resp.text)
        last = [a for a in root.xpath('//a[@class="page-link"]') if a.text == "Last"]
        if not len(last):
            return root, (page, None)
            raise Exception("Invalid html response: Page marker not found")

        lastpage = last[0].get("href").split("page_number=",1)[1].split("&",1)[0]
        return root, (page, lastpage)

    def get_contracts(self, start=1, end=None):
        page = start

        while not end or page <= end:
            root, pageResult = self._request_contract_list(page)    

            if not end:
                page, lastpage = pageResult
                page, lastpage = int(page),int(lastpage)
                end = lastpage                

            for row in root.xpath('//tr[@data-identifier-hash]'):
                cols = list(row)

                contract = {'address': row.get("data-identifier-hash").strip(),
                            'name': cols[1].text_content().strip(),
                            'compiler': cols[2].text.strip(),
                            'balance': cols[3].text.strip(),
                            'txcount': cols[4].text.strip(),
                            'settings': cols[5].text.strip(),
                            'date': cols[6].text_content().strip(),
                            }
                print(contract)
                yield contract
            page += 1
    ...
    def _request_contract_source(self, address):
        resp = self.session.get("/address/{address}/contracts".format(address=address)).text
        if "You have reached your maximum request limit for this resource. Please try again later" in resp:
            print("[[THROTTELING]]")
            raise Exception("Throtteling")
        ...
        sources = []
        for source_unit in root.xpath('//code[@class="solidity"]'):
            source = "\n".join([l.text_content() for l in source_unit])
