...
import logging
import argparse
from connector.etherscan import CeloExplorerApi
import json
...
def main():
    ...
    overwrite = False
    amount = 1000000
    break_on_dupes_threshold_initial = 400
    break_on_dupes_threshold = break_on_dupes_threshold_initial

    e = CeloExplorerApi(baseurl="https://%s"%(args.chain if not args.network else "%s.%s"%(args.network, args.chain)))
