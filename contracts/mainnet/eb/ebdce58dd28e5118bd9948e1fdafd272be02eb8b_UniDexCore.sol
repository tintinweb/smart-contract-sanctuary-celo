...
library Fabric {
    /*Vault bytecode
        ...
        0x80    - DUP1  00                    // Amount of FTM to send during call
...
contract Order {
    address public constant ETH_ADDRESS = address(0x471EcE3750Da237f93B8E339c536989b8978a438);
}
...
contract UniDexCore is Order {
    ...
    /**
     * @dev Prevent users to send Fantom directly to this contract
     */
    receive() external payable {
        require(
    ...
    /**
     * @notice Create an order from FTM to token
     * @param _data - Bytes of an FTM to token. See `encodeEthOrder` for more info
     */
    function depositEth(
        bytes calldata _data
    ) external payable {
    ...
    /**
     * @notice Get the calldata needed to create a token to token/FTM order
    ...
    /**
     * @notice Get the calldata needed to create a FTM to token order
     * @dev Returns the input data that the user needs to use to create the order
     * The _secret is used to prevent a front-running at the order execution
     * @param _module - Address of the module to use for the order execution
     * @param _inputToken - Address of the input token
    ...
    /**
     * @notice Get the vault's address of a token to token/FTM order
     * @param _module - Address of the module to use for the order execution
     * @param _inputToken - Address of the input token
     * @param _owner - Address of the order's owner
     * @param _witness - Address of the witness
     ...
     * @dev For an FTM order, the FTM will be transferred from this contract
