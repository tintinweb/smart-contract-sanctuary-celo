...
contract longhu_RNG_Celo {
    ...
    function expand(uint256 randomValue, uint n, string memory uuid) private pure returns (uint256[] memory) {
        uint256[] memory cards = new uint256[](n);
        uint256 i = 0; uint256 j = 0;
        // while(cards.length < n) {
        while(isExisted(cards, 0)) {
            uint256 expandedValue = uint256(keccak256(abi.encode(randomValue, uuid, i)));
    ...
    function randomCards(uint _cardsCount,string memory _jh, string memory _uuid) public isOwner returns(uint32) {
        ...
        reqDatas[requestId] = reqData(_cardsCount, _jh, _uuid);
        return requestId;
    }
    ...
    function emitRandomNumber(uint32 requestId) public isOwner {
        try rng.randomNumber(requestId) returns (uint256 randomNum) {
            ...
            uint256[] memory cards = expand(randomNum, data.cardsCount, data.uuid);
