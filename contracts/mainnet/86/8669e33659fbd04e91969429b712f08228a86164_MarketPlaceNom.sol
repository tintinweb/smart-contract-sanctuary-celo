...
contract MarketPlaceNom is InterfaceV2, MarketPlaceFeeAPI, MarketPlaceNFTAPI, ReentrancyGuard {
    ...
    function getListedTokenIds() external view returns (uint256[] memory) {
        uint256 querySize = _tokenIdWithListing.length();
        uint256[] memory tokenIds = new uint256[](querySize);
        for (uint256 i = 0; i < querySize; i++) {
            tokenIds[i] = _tokenIdWithListing.at(i);
        }
        ...
        for (uint256 i = 0; i < recentLists.length; i++) {
            ...
            if (_tokenListings[listing.tokenId].seller == address(0)) {
                if(listing.seller != address(0)){
                    Listing memory newList = Listing(listing.tokenId, listing.listingPrice, listing.seller);
                    _tokenListings[listing.tokenId] = newList;
                    _tokenIdWithListing.add(newList.tokenId);
                    emit NomTokenListed(
                        bytes32(newList.tokenId),
                        newList.seller,
                        newList.listingPrice
                    );
                }
            }
        }
