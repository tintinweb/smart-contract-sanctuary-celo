...
contract GatewayMint is OwnableUpgradeable, ReentrancyGuard {
    ...
    function inNetwork(
        ...
        bytes32 _nHash,
        bytes memory _sig
    ) public whenNotPaused checkMinterCap(_amount) notContract {
        require(_amount >= mininumInAmount, "_amount less than mininumInAmount");
        // Verify signature
        bytes32 signedMessageHash = hashForSignature(_networkId, _txHash, _amount, _receiver, _nHash);
        require(status[signedMessageHash] == false, "Gateway: nonce hash already spent");
        require(verifySignature(signedMessageHash, _sig), "Gateway: invalid sig");
