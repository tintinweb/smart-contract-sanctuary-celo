...
contract MarketPlaceNom is InterfaceV2, MarketPlaceFeeAPI, MarketPlaceNFTAPI, ReentrancyGuard {
     ...
     */
    function changePrice(address sender, string memory name, uint256 newPrice) external nonReentrant {
        uint256 tokenId = calculateTokenId(name);
     ...
     */
    function buyToken(address sender, string memory name, uint256 value) external payable {
        uint256 tokenId = calculateTokenId(name);
        Listing memory listing = getTokenListing(name); // Get valid listing
        require(listing.seller != address(0), "Token is not for sale"); // Listing not valid
        ...
        sendERC20(msg.sender, listing.seller, calculateSellerFee(payment_value));
        sendERC20(msg.sender, maketPlaceFeeAddress, calculateDevFee(payment_value));
        if(nftCreaterAddress != address(0) && calculateCreaterFee(payment_value) > 0){
            sendERC20(msg.sender, nftCreaterAddress, calculateCreaterFee(payment_value));
        }
        if(nftProducerAddress != address(0) && calculateProducerFee(payment_value) > 0){
            sendERC20(msg.sender, nftProducerAddress, calculateProducerFee(payment_value));
        }
    ...
    function withdrawBidForToken(address sender, string memory name) external {
        ...
        emit TokenBidWithdrawn(name, bid.bidder, bid.bidPrice);
    }
    function acceptBidForToken(address sender, string memory name, address bidder) external {
        ...
        sendERC20(address(this), msg.sender, calculateSellerFee(existingBid.bidPrice));
        sendERC20(address(this), maketPlaceFeeAddress, calculateDevFee(existingBid.bidPrice));
        if(nftCreaterAddress != address(0) && calculateCreaterFee(existingBid.bidPrice) > 0){
            sendERC20(address(this), nftCreaterAddress, calculateCreaterFee(existingBid.bidPrice));
        }
        if(nftProducerAddress != address(0) && calculateProducerFee(existingBid.bidPrice) > 0){
            sendERC20(address(this), nftProducerAddress, calculateProducerFee(existingBid.bidPrice));
        }
