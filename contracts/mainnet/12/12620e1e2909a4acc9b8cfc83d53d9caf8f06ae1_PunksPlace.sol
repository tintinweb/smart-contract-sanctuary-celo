...
/*
 * Adapted from PancakeSwap's NFT MarketPlace.
 */
...
interface IERC721EIP2981 is IERC721{
...
contract PunksPlace is ERC721Holder, Ownable, ReentrancyGuard {
    ...
    uint256 constant public MAX_ROYALTY = 1000;
    ...
    struct Collection {
        ...
        bool eip2981ish;    // in the case where the collection supports EIP-2981,
    ...
    function addCollection(
        ...
        _collections[_collection] = Collection({
            ...
            eip2981ish: _eip2981ish
        });

        emit CollectionNew(_collection, _creator, _tradingFee, _creatorFee, _eip2981ish);
    }
    ...
    function modifyCollection(
        ...
        emit CollectionUpdate(_collection, _creator, _tradingFee, _creatorFee, _eip2981ish);
    }
    ...
    function buyToken(
        ...
        (uint256 netPrice, uint256 tradingFee, uint256 creatorFee) = _calculatePriceAndFeesForCollection(
         ...
         // Handle royalties with ERC-165-compliant contracts
        // if(IERC721EIP2981(_collection).supportsInterface(_INTERFACE_ID_ERC2981)){
        //     (address _royaltiesReceiver, uint256 _priceAfterRoyalties) =
        //                 IERC721EIP2981(_collection).royaltyInfo(_tokenId, msg.value);
        //     uint256 _royalties = msg.value - _priceAfterRoyalties;
        //     netPrice -= _royalties;
        //     (bool success_, ) = payable(_royaltiesReceiver).call{value:_royalties}("");
        //     require(success_, "Transfer failed");
        // }

        // Verify if NFT collection implements EIP-2981ish
        try IERC721EIP2981(_collection).royaltyInfo(_tokenId, msg.value)
            returns (address receiver, uint256 royaltyAmount){
                uint256 _royalties;
                if(_collections[_collection].eip2981ish){
                    _royalties = msg.value - royaltyAmount; // royaltyAmount = price after royalty
                }else{
                    _royalties = royaltyAmount; // royaltyAmount = royalty
                }
                require((_royalties * DENOMINATOR) / netPrice < MAX_ROYALTY, "Royalty too high");
                netPrice -= _royalties;
