...
/**
 * @title HedgeCore Upgradable Contract for `sIMMO`
 * @author Entropyfi
...
contract Core is ICore, Initializable, UUPSUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, CoreStorageV1 {
	...
	function initialize(
		...
		require(ISToken(sToken_).index() > 0, "HC:S INV");
	...
	function initGnesisHedge(
		...
		/// 2.2 check tokens // cannot check if core address matches since the core address is the proxy address not address(this)
		hgeToken.index();
		levToken.index();
		sponsorToken.core();

		// 3 price and epoch and rebaseEndTime fetch
		/// 3.1 sToken price
		hedgeInfo.hedgeTokenPrice = sTokenPrice(); //(lastPrice is zero during the first warmup round)
		require(hedgeInfo.hedgeTokenPrice != 0, "HC:W P INV");
		/// 3.2 epoch and rebaseEndTime
		(currSTokenEpoch, , , hedgeInfo.rebaseEndTime) = _stakingContractEpoch();
		require(currSTokenEpoch != 0, "HC:IDX INV");
		require(hedgeInfo.rebaseEndTime != 0, "HC:ET INV");

		// 4. finish initialization set it to true.
		initialized = true;
	}

	//---------------------------------- core logic for user interactions ---------------------//
	/**
	 * @notice deposit sIMMO to soft hedge & leverage and get hgeToken and levToken
	 * @dev for frontend
	 */
	...
	/**
	 * @notice redeem back all available sIMMO after game paused
	...
	function withdrawAllWhenPaused() public override nonReentrant onlyInitialized whenPaused onlyWithdrawAllNotPaused isEligibleSender {
		// 1. calc hge and lev and sponsor balance
		...
		require(hgeBalance + levBalance + sponsorBalance != 0, "HC:AMNT ZR");
	...
	function startNewHedge() public override nonReentrant onlyInitialized whenNotPaused onlyAfterRebase(epoch()) {
		_startNewHedge();
	}
	...
	function _depositFor(
		...
		// 1. transfer `hgeAmount_ + levAmount_` sToken to contract
		/// sIMMO is not feeOnTransfer token so no need to check pre and post balance
		sToken.safeTransferFrom(msg.sender, address(this), hgeAmount_ + levAmount_);

		// 2. mint user game token for hedge and lev
		if (hgeAmount_ != 0) {
		...
		// 3. update mappings for user view earned profits
		userDeposited[user_] += hgeAmount_ + levAmount_;

		// 4. emit deposit event and game status
		emit Deposited(user_, hgeAmount_, levAmount_);
	}

	/**
	 * @dev burn `msg.sender`'s hgeToken and levToken and transfer sIMMO to `recipient`
	 * @param recipient_ The address which receives withrawed sIMMO
	...
	function _withdrawTo(
		...
		// 2. transfer `hgeAmount_ + levAmount_` to recipient
	...
	/**
	 * @dev sponsor `amount_` of sIMMO (sponsor will not receive rewards)
	 * @param amount_ sIMMO amount
	 */
	function _sponsorDeposit(uint256 amount_) internal {
		require(amount_ != 0, "HC:AMNT ZR");

		// 1. transfer `amount_` of sToken to contract from caller
	 ...
	 * 1. hedgeInfo: hedgeTokenPrice [√], rebaseEndTime[√]
	 * 2. token index [√]
	 * 3. currSTokenEpoch [√]
	 */
	function _startNewHedge() internal {
		...
		bool isLev;
		// 1. get result
		uint256 currSTokenPrice = sTokenPrice();
		isLev = isLevWin(isPriceRatioUp, hedgeInfo.hedgeTokenPrice, currSTokenPrice);
		...
		/// 3.3 update price
		hedgeInfo.hedgeTokenPrice = currSTokenPrice;
	...
	function _rebaseHedgeToken(
		...
		if (isLev_) {
			...
			uint256 hgeIdx = oldIdx + (actualRebasedAmount_ * PRECISION) / hgeToken.rawTotalSupply();
		...
		// 3. emit events
		emit Rebased(actualRebasedAmount_, totalRebasedAmount_);
	}
	...
	function setWhiteListContract(address contract_, bool isWhitelisted_) external onlyDAO {
		whitelistedContracts[contract_] = isWhitelisted_;
		emit ContractWhitelisted(contract_, isWhitelisted_);
	}
	...
	/**
	 * @dev [onlyDAO] range 0-8hours => deposit available window (8 - 0 hours)
	...
	/**
	 * @notice fetch epoch. data source is chainlink oracle
	 * @dev get from oracle master and the mapping entry is set to the chainlink aggregator address
	 */
	function epoch() public view returns (uint256 epoch_) {
		(epoch_, , , ) = _stakingContractEpoch();
	}

	/**
	 * @notice sIMMO price. (here are sIMMO/McUSD)
	 */
	function sTokenPrice() public view returns (uint256) {
		address oracleMaster = addressProvider.getOracleMaster();
		return IOracleMaster(oracleMaster).queryInfo(address(sToken));
	}
	...
	function hedgeCoreStatus() public view override returns (bool isUnlocked_) {
		...
		isInAllowedPriod = (block.timestamp <= hedgeInfo.rebaseEndTime - resctrictedPeriod) ? true : false;
		isRebased = _isSTokenRebased(epoch());

		isUnlocked_ = isInAllowedPriod && (!isRebased);
	}
	...
	function isSTokenRebased() external view override returns (bool) {
		return _isSTokenRebased(epoch());
	}

	function _isSTokenRebased(uint256 epoch_) internal view returns (bool) {
		return epoch_ > currSTokenEpoch ? true : false;
	}
...
interface ICore {
	...
	event ContractWhitelisted(address indexed contract_, bool indexed isWhitelisted_);
...
contract CoreStorageV1 {
	...
	modifier onlyAfterRebase(uint256 epoch_) {
		require(epoch_ > currSTokenEpoch, "HC: NOT REBASED!");
		_;
	}
