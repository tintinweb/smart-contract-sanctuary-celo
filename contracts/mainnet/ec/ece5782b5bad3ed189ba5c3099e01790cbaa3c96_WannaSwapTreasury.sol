...
contract WannaSwapTreasury is Ownable, IWannaSwapTreasury {
    ...
    function setTopTeamPercents(uint _top1TeamPercent, uint _top2TeamPercent, uint _top3TeamPercent) external onlyOwner {
        require(_top1TeamPercent.add(_top2TeamPercent).add(_top3TeamPercent) == 100, "setTopTeamPercents: BAD PERCENTS");
        top1TeamPercent = _top1TeamPercent;
        top2TeamPercent = _top2TeamPercent;
        top3TeamPercent = _top3TeamPercent;
    }

    function setRoundBlock(uint _roundBlock) external onlyOwner {
        roundBlock = _roundBlock;
    }
    ...
    function refFeeTo(address _user) external override view returns (address) {
        address result = IWannaSwapProfile(profileAddress).refFeeTo(_user);
        if (result == address(0)) {
            result = buybackAddress;
        }
        return result;
    }
    ...
    function addRound() internal {
        ...
        roundInfo.push(RoundInfo({
            ...
            closeBlock: block.number.add(roundBlock),
            totalReward: totalReward,
            isClose: false
        }));
    }

    function compareTeamVolume(uint _bid) public view returns (uint top1, uint top2, uint top3) {
        ...
        for (uint i = 1; i < tops.length - 1; i++) { // team0 is exception
            for (uint j = i + 1; j < tops.length; j++) {
                if (teamInfo[_bid][i].totalVolume < teamInfo[_bid][j].totalVolume
                    || (
                        teamInfo[_bid][i].totalVolume == teamInfo[_bid][j].totalVolume
                        && teamInfo[_bid][i].lastSwapBlock > teamInfo[_bid][j].lastSwapBlock
                    )) {
                    uint tmp = tops[i];
                    tops[i] = tops[j];
                    tops[j] = tmp;
    ...
    function updateTeam(uint _bid, uint _tid, uint _totalReward) internal {
        TeamInfo storage team = teamInfo[_bid][_tid];
    ...
    function closeRound(uint _bid) internal {
        require(_bid < roundInfo.length, "BAD ROUND");
        (uint top1, uint top2, uint top3) = compareTeamVolume(_bid);
        uint totalReward = roundInfo[_bid].totalReward;
        uint top1Reward = totalReward.mul(top1TeamPercent).div(100);
        uint top2Reward = totalReward.mul(top2TeamPercent).div(100);
        uint top3Reward = totalReward.sub(top1Reward).sub(top2Reward);
        updateTeam(_bid, top1, top1Reward);
        updateTeam(_bid, top2, top2Reward);
        updateTeam(_bid, top3, top3Reward);
        wanna.mint(address(this), totalReward);
        mintedReward = mintedReward.add(totalReward);
        roundInfo[_bid].isClose = true;
    }

    function addVolume(address _user, uint _vol) external override onlyRouter {
        if (roundInfo.length > 0) {
            uint bid = roundInfo.length.sub(1);
            if (block.number >= roundInfo[bid].closeBlock) {
                // close old round
                closeRound(bid);
                // add new round
                addRound();
                bid = roundInfo.length.sub(1);
            }
            UserInfo storage user = userInfo[bid][_user];
            user.volume = user.volume.add(_vol);
            IWannaSwapProfile profile = IWannaSwapProfile(profileAddress);
            uint tid = profile.teamid(_user);
            if (tid > 0) {
                user.tid = tid;
                TeamInfo storage team = teamInfo[bid][tid];
                team.totalVolume = team.totalVolume.add(_vol);
                team.lastSwapBlock = block.number;
            }
    ...
    function pendingWanna(address _user) public view returns (uint) {
        ...
        for (uint i = 0; i < roundInfo.length; i++) {
...
interface IWannaSwapTreasury {
    ...
    function refFeeTo(address _user) external view returns (address);
...
contract WannaSwapToken is ERC20("WannaSwap", "WANNA"), WannaSwapMintable {
    uint256 private _maxSupply = 200000000e18;
    uint256 private _initialSupply = 46000000e18;
