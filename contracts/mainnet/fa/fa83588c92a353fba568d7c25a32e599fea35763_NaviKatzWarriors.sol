...
contract NaviKatzWarriors is ERC721Enumerable, Ownable {
    ...
    event welcomeToNaviKatzWarrior(uint256 indexed id);
    event NewPriceEvent(uint256 price);
    event NewMaxElement(uint256 max);

    constructor(string memory baseURI) ERC721("NaviKatzWarrior", "NKW") {
        setBaseURI(baseURI);
    }
    ...
    function _mintAnElement(address _to, uint256 _tokenId) private {
        ...
        emit welcomeToNaviKatzWarrior(_tokenId);
    }
