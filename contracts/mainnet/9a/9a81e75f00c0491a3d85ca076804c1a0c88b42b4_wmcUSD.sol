...
contract wmcUSD is WrappedMToken {
  constructor(IERC20 _mToken, IERC20 _token)
    WrappedMToken("Wrapped mcUSD", "wmcUSD", _mToken, _token, ILendingPool(address(0x56d0Ae52f33f7C2e38E92F6D20b8ccfD7Dc318Ce)))
  {}
}
...
contract WrappedMToken is ERC20, IWERC20 {
  ...
  function wrap(uint256 underlyingAmount) public override {
    ...
    token.approve(address(lendingPool), underlyingAmount);
    lendingPool.deposit(address(token), underlyingAmount);
  ...
  function unwrap(uint256 debtAmount) public override {
    ...
    mToken.approve(address(lendingPool), toReturn);
    lendingPool.withdraw(address(token), toReturn);
