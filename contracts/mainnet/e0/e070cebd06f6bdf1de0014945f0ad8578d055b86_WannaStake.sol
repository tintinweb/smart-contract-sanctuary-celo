...
import "../interfaces/IWannaStake.sol";
import "./WannaSwapToken.sol";

contract WannaStake is Ownable, IWannaStake {
    ...
    uint public stakedWanna;
    uint public mintedWanna;
    uint public bonusWanna;
    ...
    function poolLength() external override view returns (uint) {
        return poolInfo.length;
    }

    function addPool(uint _allocPoint, uint _lockupDuration, bool _withUpdate) public onlyOwner {
        if (_withUpdate) {
            updateAllPools();
        }
    ...
    function setPool(uint _pid, uint _allocPoint, uint _lockupDuration, bool _withUpdate) public onlyOwner {
    ...
    function pendingWanna(uint _pid, address _user) public view returns (uint) {
        ...
        if (block.number > pool.lastRewardBlock && stakedWanna != 0) {
            ...
            accWannaPerShare = accWannaPerShare.add(wannaReward.mul(1e18).div(stakedWanna));
        }
    ...
    function updatePool(uint _pid, uint _bonusWanna) public {
        require(_pid < poolInfo.length, "updatePool: BAD POOL");
        PoolInfo storage pool = poolInfo[_pid];
        if (block.number <= pool.lastRewardBlock) {
            return;
        }
        if (stakedWanna == 0 || pool.allocPoint == 0) {
        ...
        uint blockCount = getBlockCount(pool.lastRewardBlock, block.number);
        uint wannaReward = blockCount.mul(wannaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        uint canMintAmount = mintedWanna.add(wannaReward) <= totalWanna ? wannaReward : totalWanna.sub(mintedWanna);
        wanna.mint(address(this), canMintAmount);
        mintedWanna = mintedWanna.add(canMintAmount);
        bonusWanna = bonusWanna.add(_bonusWanna);

        pool.accWannaPerShare = pool.accWannaPerShare.add(canMintAmount.add(_bonusWanna).mul(1e18).div(stakedWanna));
        pool.lastRewardBlock = block.number;
    }

    function addBonusWanna(uint _amount) external override {
        require(_amount > 0, "addBonusWanna: BAD AMOUNT");
        uint length = poolInfo.length;
        for (uint pid = 0; pid < length; pid++) {
            updatePool(pid, _amount.mul(poolInfo[pid].allocPoint).div(totalAllocPoint));
        }
    }
    ...
    function deposit(uint _pid, uint _amount) external override onlyEOA {
        ...
        if(_amount > 0) {
            wanna.safeTransferFrom(address(msg.sender), address(this), _amount);
            user.amount = user.amount.add(_amount);
            pool.totalLp = pool.totalLp.add(_amount);
            stakedWanna = stakedWanna.add(_amount);
    ...
    function withdraw(uint _pid, uint _amount) external override onlyEOA {
        ...
        if(_amount > 0) {
            ...
            stakedWanna = stakedWanna.sub(_amount);
            user.lastInteraction = block.timestamp;
            wanna.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accWannaPerShare).div(1e18);
...
interface IWannaStake {
    event Deposit(address indexed user, uint indexed pid, uint amount);
    event Withdraw(address indexed user, uint indexed pid, uint amount);

    function poolLength() external view returns (uint);
    function addBonusWanna(uint _amount) external;
