...
contract MoolaVault is ERC20, Ownable, ReentrancyGuard {
  ...
  function redeemReward(IERC20 _token) external nonReentrant {
    if (address(_token) == address(mToken)) {
      uint256 lockedValue = mToken.balanceOf(address(this));
      uint256 redeemable = lockedValue.sub(totalSupply());
      _token.safeTransfer(rewardRecipient, redeemable);
      emit RewardRedeemed(address(_token), redeemable);
    } else {
      uint256 redeemable = _token.balanceOf(address(this));
      _token.safeTransfer(rewardRecipient, redeemable);
      emit RewardRedeemed(address(_token), redeemable);
    }
  }
    ...
    nonReentrant
  {
    ...
    require(
      token.approve(address(lendingPool), _depositAmount),
      "Approve failed"
    );
    lendingPool.deposit(address(token), _depositAmount, address(this), 0);
    _mint(_onBehalfOf, _depositAmount);
  }
  ...
  function withdraw(uint256 _burnAmount, address _to) external nonReentrant {
    ...
    lendingPool.withdraw(address(token), _burnAmount, _to);
  }
}
