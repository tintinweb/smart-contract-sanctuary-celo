...
import "src/interfaces/IMoolaLendingPool.sol";
import "src/interfaces/IStableToken.sol";
import "src/staking/StakedERC4626Upgradeable.sol";
import "src/interfaces/IRegistry.sol";

import "src/staking/SpiralsCUSDVault.sol";
import "src/vaults/ImpactVault.sol";
import "src/vaults/SpiralsCUSDImpactVault.sol";
import "src/staking/migration/IMigrationToImpactVault.sol";
import {IERC20Upgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC20/IERC20Upgradeable.sol";

contract SpiralsCUSDVaultMigration is
    IMigrationToImpactVault,
    SpiralsCUSDVault
{
    ...
    function transferAssetsToImpactVaultForAddress(
        ...
        uint256 cUSDStaked = balanceOf(_address);
        uint256 totalCUSD = cUSDStaked + _yield;
        if (totalCUSD == 0) {
            return;
        }

        // withdraw from Moola LP (mcUSD -> cUSD)
        c_moolaLendingPool.withdraw(asset(), totalCUSD, address(this));
        _burn(_address, cUSDStaked);

        // deposit into new vault
        IERC20Upgradeable(asset()).approve(address(_vault), totalCUSD);
        SpiralsCUSDImpactVault cUSDVault = SpiralsCUSDImpactVault(
            payable(address(_vault))
        );
        cUSDVault.depositWithYield(cUSDStaked, _address, _yield);
        ...
        // This event will decrease cUSD staked count on Spirals subgraph
        // but as long as subgraph is handling "Deposit" event appropriately
        // on the new vault then we are good.
        emit Withdraw(msg.sender, _address, _address, cUSDStaked, cUSDStaked);
    }
...
/**
 * @title SpiralsCUSDImpactVault
 * @author @douglasqian @no40
 * @notice Implementation of ImpactVault on the Celo USD stablecoin (cUSD).
 *   cUSD deposited is deposited in Moola Market's cUSD lending pool as
 ...
 *   receives a rebasing token (mcUSD) that can be redeemed 1-to-1 for cUSD.
 */
contract SpiralsCUSDImpactVault is ImpactVault {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event Receive(address indexed sender, uint256 indexed amount);
    event DependenciesUpdated(
        address indexed moolaLendingPool,
        address celoRegistry
    );

    IRegistry internal c_celoRegistry;
    IMoolaLendingPool internal c_moolaLendingPool;
    IERC4626Upgradeable internal c_wmcUSD;
    ...
    function initialize(
        ...
        setDependencies(_moolaLendingPoolAddress, _celoRegistryAddress);

        // Ensures that `_moolaLendingPoolAddress` has been sanitized.
        ...
        __ERC20_init("Spirals cUSD Vault Token", "spcUSD");
        __ImpactVault_init(
            cUSD,
            IERC20Upgradeable(c_wmcUSD),
            _impactVaultManagerAddress
        );
    }

    /**
     * @notice Sets dependencies on contract (Moola contract addresses).
     */
    function setDependencies(
        ...
        c_moolaLendingPool = IMoolaLendingPool(_moolaLendingPoolAddress);
        c_celoRegistry = IRegistry(_celoRegistryAddress);

        // Keep this here becaue "getStableToken" has a dependency on
        // "c_celoRegistry" being set.
        require(
            IMoolaLendingPool(_moolaLendingPoolAddress)
                .getReserveData(address(getStableToken()))
                .aTokenAddress != address(0),
            "INVALID_LENDING_POOL"
        );
        emit DependenciesUpdated(
            _moolaLendingPoolAddress,
            _celoRegistryAddress
        );
    }

    /**
     * @dev Deposit cUSD into Moola lending pool and convert mcUSD to wmcUSD.
     */
    function _stake(uint256 _amount) internal virtual override {
        // Using SafeERC20Upgradeable
        // slither-disable-next-line unused-return
        asset.approve(address(c_moolaLendingPool), _amount);

        uint256 mcUSDBefore = getMCUSD().balanceOf(address(this));
        c_moolaLendingPool.deposit(address(asset), _amount, address(this), 0);
        uint256 mcUSDReceived = getMCUSD().balanceOf(address(this)) -
            mcUSDBefore;

        // Convert mcUSD to wmcUSD.
        getMCUSD().approve(address(c_wmcUSD), mcUSDReceived);
        c_wmcUSD.deposit(mcUSDReceived, address(this));
    }

    /**
     * @dev Withdraws cUSD from wmcUSD and then converts it to mcUSD from
     * Moola lending pool into cUSD to send to receiver.
     */
    function _withdraw(address _receiver, uint256 _amount)
        ...
        override
    {
        // Withdraw mcUSD assets to this vault.
        uint256 mcUSDBefore = getMCUSD().balanceOf(address(this));
        c_wmcUSD.withdraw(_amount, address(this), address(this));
        uint256 mcUSDReceived = getMCUSD().balanceOf(address(this)) -
            mcUSDBefore;

        // Burn mcUSD to withdraw cUSD to receiver.
        // withdraw here returns _amount, return value not needed
        // slither-disable-next-line unused-return
        c_moolaLendingPool.withdraw(address(asset), mcUSDReceived, _receiver);
    }
    ...
    {
        return c_wmcUSD.convertToAssets(_amountYieldAsset);
    }
    ...
    {
        return c_wmcUSD.convertToShares(_amountAsset);
    }

    /**
     * @dev Returns the StableToken contract (cUSD).
     */
    function getStableToken() internal view returns (IERC20Upgradeable) {
        address stableTokenAddr = c_celoRegistry.getAddressForStringOrDie(
            "StableToken"
        );
    ...
    /**
     * @dev Returns the Moola cUSD AToken contract (mcUSD).
     */
    function getMCUSD() internal view returns (IERC20Upgradeable) {
        return
            IERC20Upgradeable(
                c_moolaLendingPool
                    .getReserveData(address(getStableToken()))
...
/**
 * @dev Extension of ERC-4626 where the underlying asset is staked.
 * This contract tracks the staked asset contract as well, and handles
 * conversions between the two assets.
 */

abstract contract StakedERC4626Upgradeable is
    ERC4626Upgradeable,
    PausableUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    IERC20MetadataUpgradeable private _stakedAsset;

    /**
     * @dev Set the underlying asset and staked asset contracts.
     * These must be an ERC20-compatible contracts (ERC20 or ERC777).
     */
    function __StakedERC4626_init(IERC20MetadataUpgradeable stakedAsset_)
        internal
        onlyInitializing
    {
        __StakedERC4626_init_unchained(stakedAsset_);
    }

    function __StakedERC4626_init_unchained(
        IERC20MetadataUpgradeable stakedAsset_
    ) internal onlyInitializing {
        _stakedAsset = stakedAsset_;
    }
    ...
    /**
     * @dev Extension of original "deposit" method with a post-deposit hook
     * to convert vault assets into staked assets.
     */
        ...
        returns (uint256 shares)
    {
        ...
        stake(convertToAssets(shares));
        return shares;
    }

    /**
     * @dev Post-deposit hook to convert asset to stakedAsset in vault.
     */
    function stake(uint256 assets) internal virtual;

    /**
     * @dev Common code for withdraw flow.
     */
    function _withdraw(
        address caller,
        address receiver,
        ...
        _checkCanWithdraw(caller, owner, receiver, shares, assets);

        // Burn shares before subsequent withdraws. Do this before other
        // steps to protect from reentrant attacks.
        _burn(owner, shares);
        assets = _withdrawFromStakedAsset(convertAssetsToStaked(assets));
        _sendAssetToReceiver(owner, receiver, assets);

        emit Withdraw(caller, receiver, owner, assets, shares);
    }

    /**
     * @dev Revert if withdrawal invalid for given inputs. Optional.
     */
    ...
    /**
     * @dev Withdraw staked asset into asset in vault. Reverts on error.
     */
    ...
    /**
     * @dev Post-withdraw hook to send converted assets to receiver
     */
    ...
    /**
     * @dev Returns total value of staked ERC4626 vault, denominated in `_asset`
     */
        ...
        returns (uint256 assets)
    {
        return
            ...
            convertStakedToAssets(_stakedAsset.balanceOf(address(this)));
    }

    /**
     * @dev Returns a breakdown of asset vs. staked asset stored in the vault,
     * value denominated in `_asset`
     */
    function getVaultBreakdown()
        external
        view
        virtual
...
/// @title SpiralsCUSDVault
/// @author @douglasqian @no40
/// @notice This is a modification of the EIP-4626 tokenized vault standard
/// for yield-bearing tokens where the yield accrued on cUSD via a lending protocol
/// is held by the contract, giving the user a derivative token to redeem them at a
/// later date.
///
/// DEPRECATED: see "SpiralsCUSDImpactVault"
contract SpiralsCUSDVault is StakedERC4626Upgradeable {
    event Receive(address indexed sender, uint256 indexed amount);
    event DependenciesUpdated(
        address indexed moolaLendingPool,
        address celoRegistry
    ...
    function initialize(
        ...
        setDependencies(_moolaLendingPoolAddress, _celoRegistryAddress);
        // Ensures that `_moolaLendingPoolAddress` has been sanitized.
        address cUSDAddress = address(getStableToken());
        __ERC20_init("Spirals cUSD Vault Token", "spcUSD");
        __ERC4626_init(IERC20MetadataUpgradeable(cUSDAddress));
        __StakedERC4626_init(
            IERC20MetadataUpgradeable(
                c_moolaLendingPool.getReserveData(cUSDAddress).aTokenAddress
            )
        );
    }
     ...
     * @notice Sets dependencies on contract (Moola contract addresses).
     */
    // lendingPoolAddrProvider 0xD1088091A174d33412a968Fa34Cb67131188B332
    ...
    /**
     * @dev Deposit cUSD into Moola cUSD lending pool.
     */
    function stake(uint256 assets) internal virtual override {
        // Approve cUSD from contract -> Moola LP
        SafeERC20Upgradeable.safeApprove(
            IERC20Upgradeable(asset()),
            address(c_moolaLendingPool),
            assets
        );
        c_moolaLendingPool.deposit(asset(), assets, address(this), 0);
    }

    /**
     * @dev Withdraw cUSD from Moola lending pool.
     */
    function _withdrawFromStakedAsset(uint256 stakedAssets)
        internal
        virtual
        ...
        returns (uint256 assets)
    {
        // Withdraw from Moola lending pool directly into "receiver" in
        // "_sendAssetToReceiver"
        return convertStakedToAssets(stakedAssets);
    }

    /**
     * @dev Send cUSD to receiver.
     */
    function _sendAssetToReceiver(
        address,
        address receiver,
        ...
        c_moolaLendingPool.withdraw(
            asset(),
            convertAssetsToStaked(assets),
            receiver
        );
    }
    ...
    function _convertToShares(uint256 assets, MathUpgradeable.Rounding)
        ...
        returns (uint256 shares)
    {
        return assets; // # assets = # shares for rebasing token
    }
    ...
    function _convertToAssets(uint256 shares, MathUpgradeable.Rounding)
        ...
        returns (uint256 assets)
    {
        return shares; // # assets = # shares for rebasing token
    }
    ...
    /// @dev Returns a StableToken.sol interface for interacting with the smart contract.
    function getStableToken() internal view returns (IStableToken) {
        address stableTokenAddr = c_celoRegistry.getAddressForStringOrDie(
            "StableToken"
        );
        return IStableToken(stableTokenAddr);
    }
}
 ...
 */
abstract contract IStableToken is ERC20 {
    function burn(uint256) external virtual returns (bool);

    function setInflationParameters(uint256, uint256) external virtual;

    function valueToUnits(uint256) external view virtual returns (uint256);

    function unitsToValue(uint256) external view virtual returns (uint256);
 ...
 * @author Aave
 **/
interface IMoolaLendingPoolAddressesProvider {
...
import {IMoolaLendingPoolAddressesProvider} from "./IMoolaLendingPoolAddressesProvider.sol";
import {DataTypes} from "./MoolaDataTypes.sol";
...
interface IMoolaLendingPool {
    ...
    function getAddressesProvider()
        external
        view
        returns (IMoolaLendingPoolAddressesProvider);
