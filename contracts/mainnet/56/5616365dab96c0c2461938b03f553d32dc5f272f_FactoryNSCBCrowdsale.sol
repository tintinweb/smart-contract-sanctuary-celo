...
import "./NSCCrowdsale/NSCBonusableCrowdsale.sol";
import "./NSCCrowdsale/NSCBonusableDatesChangeableCrowdsale.sol";
import "../IController.sol";

contract FactoryNSCBCrowdsale is ReentrancyGuard {
    ...
    function deployNonSoftCappableBonusableCrowdsale(
        ...
        NonSoftCappableBonusableCrowdsale crowdsale = new NonSoftCappableBonusableCrowdsale(_token, _tokenDecimals, _duration);
    ...
    function deployNonSoftCappableBonusableDatesChangeableCrowdsale(
        address[2] calldata tokenToPayAndOwner,
        address _token, uint256 _tokenDecimals,
        uint256 _duration,
        address[] calldata _tokens, uint256[] calldata _rates,
        ...
        IERC20(tokenToPayAndOwner[0]).safeTransferFrom(msg.sender, CONTROLLER.feeReceiver(), _price);
        require(_tokens.length == _rates.length, "Invalid parameters");
        NonSoftCappableBonusableDatesChangeableCrowdsale crowdsale = new NonSoftCappableBonusableDatesChangeableCrowdsale(_token, _tokenDecimals, _duration);
        crowdsale.setLimits(_limits[0], _limits[1]);
        crowdsale.setBonus(_bonus[0], _bonus[1]);
        for (uint256 i; i < _tokens.length; i++) {
            crowdsale.addPaymentMethod(_tokens[i], _rates[i]);
...
contract NonSoftCappableBonusableCrowdsale is BonusableCrowdsale {
    using SafeERC20 for IERC20;
...
contract NonSoftCappableBonusableDatesChangeableCrowdsale is BonusableCrowdsale, DatesChangeableCrowdsale {
    using SafeERC20 for IERC20;
    ...
    function _calculate(uint256 amountToPay, uint256 _rate) internal override(CrowdsaleBase, BonusableCrowdsale) returns(uint256) {
