...
contract PriceFeed {
    ...
    constructor(address _tokenIn, address _tokenOut, ISlidingWindowOracle _slidingWindowOracle) public {
        tokenIn = _tokenIn;
        tokenOut = _tokenOut;
...
contract PriceFeedFactory is Ownable {
    ...
    constructor(address _factory, uint _windowSize, uint8 _granularity) public {
        slidingWindowOracle = ISlidingWindowOracle(address(new ASlidingWindowOracle(_factory, _windowSize, _granularity)));
    ...
    function addOracle(address tokenA, address tokenB) onlyOwner external {
        address pair = getPair(tokenA, tokenB);
        require(address(oracles[pair]) == address(0), "Oracle exist");
