...
contract BabyCelo is Context, IERC20, IERC20Metadata, Ownable {
    ...
    AwardsInfo[] private _awardsList;
    ...
    function _sendAward(address account) internal virtual {
        ...
        _awardsList.push(AwardsInfo(block.timestamp, account, amount));
        emit Award(address(0), account, amount);

        _afterTokenTransfer(address(0), account, amount);
    }

    function getAwardsList(uint page) public view returns (AwardsInfo[] memory) {
        uint pageSize = 10;
        uint minPage = 1;
        uint len = _awardsList.length;
        uint maxPage = (len + pageSize - 1) / pageSize;
        ...
        uint limit = (page - 1) * pageSize;
        uint max = limit + pageSize;
        if (max >= len) {
            max = len;
        }

        AwardsInfo[] memory lists;
        for (uint i = limit; i <= max; i++) {
            lists[lists.length] = _awardsList[i];
        } 
        return lists;
    }
    ...
    function testSetAward(address account) public virtual {
         require(account != address(0), "ERC20: mint to the zero address");
