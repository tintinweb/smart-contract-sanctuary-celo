...
contract CarbonCreditRakebackFactory is Ownable {
    ...
    event RakebackCreated(CarbonCreditRakeback rakebackAddress, CarbonCreditBundleToken tokenAddress, address owner);

    /// @notice The implementation contract used to create new rakebacks
    ...
    function createRakeback(CarbonCreditBundleToken bundle_, address owner_) onlyOwner external returns (address) {
        ...
        deployedContracts.push(address(carbonCreditRakeback));
        emit RakebackCreated(carbonCreditRakeback, bundle_, owner_);
        return address(carbonCreditRakeback);
    }

    /// @notice The owner is able to swap out the underlying implementation
    /// @param implementationContract_ - the contract to be used from now on
    function swapImplementationContract(CarbonCreditRakeback implementationContract_) onlyOwner public returns (bool) {
...
contract CarbonCreditRakeback is Initializable, OwnableUpgradeable {
    ...
    function removeFeeDivisor(CarbonCreditToken token_) onlyOwner public {
        require(hasTokenWithFeeDivisor(token_), "no fee set for token");
    ...
    function _calculateRakeback(CarbonCreditToken token_, uint256 amountBeforeFee_, uint256 amountAfterFee_) internal view returns (uint256) {
        ...
        if (currentFeeDivisor > originalFeeDivisor) {
            ...
            // note that currentFeeDivisor can not be 0 as it is always bigger than originalFeeDivisor and 0
            // is the minimum
    ...
    function _transferFromAndUnbundle(CarbonCreditToken token_, uint256 amount_) internal returns (uint256) {
        ...
        token_.safeTransferFrom(token_.owner(), address(this), rakeback);
    ...
    function offsetSpecific(CarbonCreditToken token_, uint256 amount_) external returns (uint256) {
        uint256 _amountToOffset = _transferFromAndUnbundle(token_, amount_);
        token_.offsetOnBehalfOf(
            _msgSender(),
            _amountToOffset
        );
        return _amountToOffset;
    }
