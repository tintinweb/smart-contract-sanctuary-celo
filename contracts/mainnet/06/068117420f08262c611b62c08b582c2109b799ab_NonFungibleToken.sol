...
abstract contract Whitelist {
    ...
    function _getMessageHash(address _to, uint256 _amount) internal pure returns (bytes32) {
    ...
    function _verify(address _to, uint256 _amount, bytes memory _signature) internal view returns (bool) {
        bytes32 messageHash = _getMessageHash(_to, _amount);
...
contract NonFungibleToken is ERC721URIStorage, Ownable, Pausable, Whitelist {
    ...
    function whitelistMint(uint256 _maxAmount, bytes memory _signature) external payable whenNotPaused {
        require(_verify(msg.sender, _maxAmount, _signature), "invalid arguments or not whitelisted");
        ...
        require(_whitelistMinted[msg.sender] <= _maxAmount, "over account whitelist limit");
    }
