...
contract celoLaunch is Context, IERC20, Ownable {
    ...
    address public _buyBackWallet = address(0xACBce62CD53A266ff8c587349D02e2b963096db7);
    ...
    event BuyBackWallet(address wallet);
    ...
    function swapTokensForEth(uint256 tokenAmount) private {
        ...
        try
            ...
            tokenAmount,
            amountOutMins[path.length - 1], // accept any amount of CELO
            path,
            _buyBackWallet, // BuyBack Wallet
            block.timestamp
            )
        {} catch {
            emit Log("external call failed");
        }
        _CELO.transferFrom(_buyBackWallet,address(this),amountOutMins[path.length - 1]);
    ...
    function swapETHForTokens(uint256 amount) private {
        ...
        // make the swap
        try
            uniswapV2Router.swapExactTokensForTokensSupportingFeeOnTransferTokens(amount,amountOutMins[path.length - 1].mul(95).div(10**2), path, burnAddress, block.timestamp)
        {} catch {
            emit Log("external call failed");
        }
    ...
    function setNewRouter(address newRouter) external onlyOwner() {
        ...
        address get_pair = IUniswapV2Factory(_newRouter.factory()).getPair(address(this), CELO);
        if (get_pair == address(0)) {
            uniswapV2Pair = IUniswapV2Factory(_newRouter.factory()).createPair(address(this), CELO);
        }
        else {
    ...
    function setBuyBackWallet(address buybackWalet) public onlyOwner {
        ...
        emit BuyBackWallet(buybackWalet);
    }
