...
contract CheeBandOracle is PriceOracle {
    ...
    mapping(address => AssetsPath) internal feeds;
    event PricePosted(address asset, uint previousPriceMantissa, uint requestedPriceMantissa, uint newPriceMantissa);
    event NewAdmin(address oldAdmin, address newAdmin);
    event FeedSet(address token, string base, string quote);
    ...
    function getPrice(CToken cToken) internal view returns (uint price) {
        ...
        if (prices[address(token)] != 0) {
            ...
            (string memory base, string memory quote) = getFeed(address(token));
            price = getBandPrice(base, quote);
        }
       
        return price;
    }

    function getBandPrice(string memory base, string memory quote) internal view returns (uint) {
        // Band USD-denominated feeds store answers at 18 decimals
        IStdReference.ReferenceData memory data = ref.getReferenceData(base, quote);

        return uint(data.rate);
    }
    ...
    function setFeed(address token, string calldata base, string calldata quote) external onlyAdmin() {
        require(token != address(0), "invalid feed");
        require(bytes(base).length > 0, "invalid base");
        require(bytes(quote).length > 0, "invalid quote");
        emit FeedSet(token, base, quote);
        feeds[token] = AssetsPath(base, quote);
    }

    function getFeed(address token) public view returns (string memory, string memory) {
        AssetsPath memory assetsPath = feeds[token];
        return (assetsPath.base, assetsPath.quote);
    }
