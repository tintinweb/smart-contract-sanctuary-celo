...
contract RegistryUniswapV2 {
    ...
    function findPairs(
        ...
        Pair[] memory temp = new Pair[](limit);
        uint found = 0;
        for (uint i = offset; i < allPairsUpTo; i++) {
            ...
            if (isOnWhitelist) {
                ...
                temp[found++] = Pair(uniPair, token0, token1, State(reserve0, reserve1));
            }
        }

        result = new Pair[](found);
        for (uint i = 0; i < found; i++) {
            Pair memory t = temp[i];
            result[i] = Pair(t.pair, t.token0, t.token1, State(t.state.reserve0, t.state.reserve1));
        }
    }
    ...
    function refreshPairs(
        ...
        for (uint i = 0; i < pairs.length; i++) {
            ...
            result[i] = State(reserve0, reserve1);
        }
    }
}