...
contract PairUniswapV3 is ISwappaPairV1, IUniswapV3SwapCallback {
    ...
    function getSpotTicks(IUniswapV3Pool pool)
        ...
        returns (
            ...
            TickLens.PopulatedTick memory activeTick,
            TickLens.PopulatedTick[] memory populatedTicks
        )
    {
        return getSpotTicksIfChanged(pool, TickMath.MIN_TICK - 1);
    }

    function getSpotTicksIfChanged(IUniswapV3Pool pool, int24 previousTick)
        public
        view
        returns (
            uint160 sqrtPriceX96,
    ...
    {
        (sqrtPriceX96, liquidity, tickBitmapIndex, activeTick) = TickLens
            .getSpotInfo(pool);

        if (activeTick.tick == previousTick) {
            // the active tick did not change, do not need to refresh the populated ticks
            populatedTicks = new TickLens.PopulatedTick[](0);
        } else {
...
library TickLens {
    ...
    function getSpotInfo(IUniswapV3Pool pool)
        ...
        returns (
            ...
            uint128 liquidity,
            int16 tickBitmapIndex,
            PopulatedTick memory activeTick
        )
    {
        ...
        // get active tick liquidity gross and net
        activeTick.tick = tick;
        (activeTick.liquidityGross, activeTick.liquidityNet, , , , , , ) = pool
            .ticks(tick);
    }
