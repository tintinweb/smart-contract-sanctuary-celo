...
interface IStrategy {
    // Total alphabeta tokens managed by strategy
    function tokenLockedTotal() external view returns (uint256);

    // Sum of all shares of users to wantLockedTotal
    function sharesTotal() external view returns (uint256);

    // Main alphabeta token compounding function
...
abstract contract AlphaBetaToken is ERC20 {
    function mint(address _to, uint256 _amount) public virtual;
}

contract AlphaBetaFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // Whenever a user deposits or withdraws alphabeta tokens to a pool. Here's what happens:
    ...
    struct PoolInfo {
        IERC20 alphabeta; // Address of the alphabeta token.
        ...
        address strat; // Strategy address that will auto compound alphabeta tokens
    }

    address public immutable ALPHABETA;
    ...
    event SetTokenPerBlock(uint256 oldTokenPerBlock, uint256 tokenPerBlock);

    constructor(address _AlphaBetaToken, uint256 _startBlock) public {
        ALPHABETA = _AlphaBetaToken;
    ...
    function getMultiplier(uint256 _from, uint256 _to) public view returns (uint256) {
        if (IERC20(ALPHABETA).totalSupply() >= maxSupply) {
            return 0;
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        AlphaBetaToken(ALPHABETA).mint(
            owner(),
            tokenReward.mul(ownerTokenReward).div(1000)
        );
        AlphaBetaToken(ALPHABETA).mint(address(this), tokenReward);
    ...
    // AlphaBeta tokens moved from user -> TokenFarm (Token allocation) -> Strat (compounding)
    function deposit(uint256 _pid, uint256 _alphabetaAmt) external nonReentrant {
        updatePool(_pid);
        PoolInfo storage pool = poolInfo[_pid];
        ...
        if (_alphabetaAmt > 0) {
            pool.alphabeta.safeTransferFrom(
                address(msg.sender),
                address(this),
                _alphabetaAmt
            );

            pool.alphabeta.safeIncreaseAllowance(pool.strat, _alphabetaAmt);
            uint256 sharesAdded = IStrategy(poolInfo[_pid].strat).deposit(_alphabetaAmt);
            user.shares = user.shares.add(sharesAdded);
        }
        user.rewardDebt = user.shares.mul(pool.accTokenPerShare).div(1e12);
        emit Deposit(msg.sender, _pid, _alphabetaAmt);
    }

    // Withdraw LP tokens from MasterChef.
    function withdraw(uint256 _pid, uint256 _alphabetaAmt) public nonReentrant {
        updatePool(_pid);
        ...
        // Withdraw alphabeta tokens
        uint256 amount = user.shares.mul(tokenLockedTotal).div(sharesTotal);
        if (_alphabetaAmt > amount) {
            _alphabetaAmt = amount;
        }
        if (_alphabetaAmt > 0) {
            uint256 sharesRemoved = IStrategy(poolInfo[_pid].strat).withdraw(_alphabetaAmt);
            ...
            uint256 alphabetaBal = IERC20(pool.alphabeta).balanceOf(address(this));
            if (alphabetaBal < _alphabetaAmt) {
                _alphabetaAmt = alphabetaBal;
            }
            pool.alphabeta.safeTransfer(address(msg.sender), _alphabetaAmt);
        }
        user.rewardDebt = user.shares.mul(pool.accTokenPerShare).div(1e12);
        emit Withdraw(msg.sender, _pid, _alphabetaAmt);
    }
    ...
    function emergencyWithdraw(uint256 _pid) external nonReentrant {
        ...
        pool.alphabeta.safeTransfer(address(msg.sender), amount);
    ...
    function safeTokenTransfer(address _to, uint256 _TokenAmt) internal {
        uint256 TokenBal = IERC20(ALPHABETA).balanceOf(address(this));
        bool transferSuccess = false;

        if (_TokenAmt > TokenBal) {
            transferSuccess = IERC20(ALPHABETA).transfer(_to, TokenBal);
        } else {
            transferSuccess = IERC20(ALPHABETA).transfer(_to, _TokenAmt);
        }
    ...
    */
    ...
    // XXX DO NOT add the same LP token more than once. Rewards will be messed up if you do. (Only if alphabeta tokens are stored here.)
    function addPool(
        uint256 _allocPoint,
        IERC20 _alphabeta,
        bool _withUpdate,
        ...
        require(!availableAssets[_alphabeta], "Can't add another pool of same asset");
        if (_withUpdate) {
            massUpdatePools();
        }
        ...
        availableAssets[_alphabeta] = true;
    }
