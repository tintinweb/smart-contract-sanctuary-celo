...
contract CarbonCreditPermissionListFactory is Ownable {
    ...
    /// @notice Emitted when a new permission list has been created by this factory
    /// @param permissionListAddress - the address of the freshly deployed contract
    event PermissionListCreated(address permissionListAddress, address owner);

    /// @notice The implementation contract used to create new lists
    ...
    function createCarbonCreditPermissionList(string memory name_, address owner_) onlyOwner external returns (address) {
        ...
        deployedContracts.push(address(permissionList));
        emit PermissionListCreated(address(permissionList), owner_);
        return address(permissionList);
    }

    /// @notice The owner is able to swap out the underlying list implementation
    /// @param implementationContract_ - the contract to be used from now on
    function swapImplementationContract(ICarbonCreditPermissionList implementationContract_) onlyOwner public returns (bool) {
