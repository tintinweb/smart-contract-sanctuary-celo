...
contract CeloDexTokenLocker is ContextUpgradeSafe, ITokenLocker {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    address public governance;

    address public clx = address(0x0000000000000000000000000000000000000000);
    ...
    function initialize(address _clx, uint256 startReleaseTime_, uint256 endReleaseTime_) public initializer {
        __Context_init();
        require(endReleaseTime_ > startReleaseTime_, "MTokenLocker: endReleaseTime_ is before startReleaseTime_");
        clx = _clx;
    ...
    modifier onlyGovernance() {
        require(msg.sender == governance, "MTokenLocker: !governance");
        _;
    }
    ...
    function editLocker(uint256 startReleaseTime_, uint256 endReleaseTime_) external onlyGovernance {
        require(_startReleaseTime > block.timestamp && startReleaseTime_ > block.timestamp, "MTokenLocker: late");
        require(endReleaseTime_ > startReleaseTime_, "MTokenLocker: invalid _endReleaseTime");
        _startReleaseTime = startReleaseTime_;
        _endReleaseTime = endReleaseTime_;
        emit EditLocker(startReleaseTime_, endReleaseTime_);
    }
    ...
    function lock(address _account, uint256 _amount) external override {
        require(block.timestamp < _startReleaseTime, "MTokenLocker: no more lock");
        require(_account != address(0), "MTokenLocker: no lock to address(0)");
        require(_amount > 0, "MTokenLocker: zero lock");

        IERC20(clx).safeTransferFrom(msg.sender, address(this), _amount);
    ...
    function canUnlockAmount(address _account) public override view returns (uint256) {
        if (block.timestamp < _startReleaseTime) {
            ...
            uint256 _releasedTime = block.timestamp.sub(_startReleaseTime);
            uint256 _totalVestingTime = _endReleaseTime.sub(_startReleaseTime);
    ...
    function claimUnlockedFor(address _account) public {
        require(block.timestamp > _startReleaseTime, "MTokenLocker: still locked");
        require(_locks[_account] > _released[_account], "MTokenLocker: no locked");

        uint256 _amount = canUnlockAmount(_account);
        IERC20(clx).safeTransfer(_account, _amount);
    ...
    function governanceRecoverUnsupported(address _token, address _to, uint256 _amount) external onlyGovernance {
        require(_token != clx || IERC20(clx).balanceOf(address(this)).sub(_amount) >= _totalLock, "MTokenLocker: Not enough locked amount left");
