...
contract HAREM is ERC20('HAREM Token', 'HAREM') {
...
// MasterChef is the master of STROM. He can make harem and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once harem is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 accHaremPerShare;   // Accumulated Yieldes per share, times 1e18. See below.
    ...
    // The Harem TOKEN!
    HAREM public immutable Harem;
    address public devAddress;
	address public treasuryAddress;
    address public feeAddress;
    

    // Harem tokens created per block.
    uint256 public HaremPerBlock = 10 ether;
    
    uint256 constant MAX_HAREM_PERBLOCK= 10 ether;
    ...
    // The block number when Harem mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 HaremPerBlock);
    ...
    // Update the given pool's Harem allocation point and withdraw fee. Can only be called by the owner.
    ...
    // View function to see pending Haremes on frontend.
    function pendingHarem(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            ...
            uint256 HaremReward = multiplier.mul(HaremPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accHaremPerShare = accHaremPerShare.add(HaremReward.mul(1e18).div(lpSupply));
        }
        return user.amount.mul(accHaremPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 HaremReward = multiplier.mul(HaremPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Harem.mint(devAddress, HaremReward.mul(10).div(100));
		Harem.mint(treasuryAddress, HaremReward.mul(10).div(100));
        Harem.mint(address(this), HaremReward);
        // Automatically burn 2% of minted tokens
        Harem.mint(BURN_ADDRESS, HaremReward.mul(50).div(1000));
        pool.accHaremPerShare = pool.accHaremPerShare.add(HaremReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accHaremPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accHaremPerShare).div(1e18);
    ...
    // Safe Harem transfer function, just in case if rounding error causes pool to not have enough Harem.
    ...
    function updateEmissionRate(uint256 _HaremPerBlock) external onlyOwner {
        require(_HaremPerBlock<=MAX_HAREM_PERBLOCK,"Too high");
        massUpdatePools();
        HaremPerBlock = _HaremPerBlock;
        emit UpdateEmissionRate(msg.sender, _HaremPerBlock);
    }
