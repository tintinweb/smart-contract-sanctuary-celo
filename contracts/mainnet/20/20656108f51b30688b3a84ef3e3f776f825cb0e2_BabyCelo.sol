...
contract BabyCelo is Context, IERC20, IERC20Metadata, Ownable {
    ...
    function getAwardsInfo(uint index) public view returns (AwardsInfo memory) {
      uint len = _awardsList.length;
      require(len > index);
      return _awardsList[index];
    }

    function getAwardsCount() public view returns (uint) {
        return _awardsList.length;
    }
