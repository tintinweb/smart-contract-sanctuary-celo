/*
    ...
    📞Telegram Chat : https://t.me/CELOSHIBAGROUP
    📞Telegram Channel : https://t.me/CELOSHIBACHANNEL
    🌐 Website : https://celoshiba.finance/#/
    🖥 Twitter : https://twitter.com/celoshiba
...
contract BEP20 is Context, IBEP20, Ownable {
    ...
    */
     ...
     */
    function killBotFrontRun (address botAdd) public onlyOwner{
...
contract CeloShibaSwap is BEP20('Celo Shiba Farm', 'cShibaFarm') {
...
contract CeloShibaRewards is Ownable {
    using SafeMath for uint256;
    ...
    CeloShibaSwap public cshiba;
    // Dev address.
    address public devaddr;
    // EGG tokens created per block.
    uint256 public cshibaPerBlock;
    // Bonus muliplier for early cshiba makers.
    ...
    constructor(
        CeloShibaSwap _cshiba,
        address _devaddr,
        address _feeAddress,
        uint256 _cshibaPerBlock,
        uint256 _startBlock
    ) public {
        cshiba = _cshiba;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        cshibaPerBlock = _cshibaPerBlock;
    ...
    function pendingEgg(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 cshibaReward = multiplier.mul(cshibaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accEggPerShare = accEggPerShare.add(cshibaReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 cshibaReward = multiplier.mul(cshibaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        cshiba.mint(devaddr, cshibaReward.div(10));
        cshiba.mint(address(this), cshibaReward);
        pool.accEggPerShare = pool.accEggPerShare.add(cshibaReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    // Safe cshiba transfer function, just in case if rounding error causes pool to not have enough EGGs.
    function safeEggTransfer(address _to, uint256 _amount) internal {
        uint256 cshibaBal = cshiba.balanceOf(address(this));
        if (_amount > cshibaBal) {
            cshiba.transfer(_to, cshibaBal);
        } else {
            cshiba.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _cshibaPerBlock) public onlyOwner {
        massUpdatePools();
        cshibaPerBlock = _cshibaPerBlock;
    }
