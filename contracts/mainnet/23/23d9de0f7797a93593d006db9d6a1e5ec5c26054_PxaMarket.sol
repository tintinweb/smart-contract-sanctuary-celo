...
contract PxaMarket is Pausable, IPxaMarket, AccessControl, ERC721Holder {
  ...
  function orderIds() external view returns (uint256[] memory _ids) {
    return _orderIds;
  }

  function totalWeight() external view returns (uint256) {
    return IPWS(_pwsAddress).totalWeightOf(_orderIds);
  }
  ...
  function _shareIncome(uint256 value) internal {
    if (_orderIds.length == 0) {
      _addIncome(value);
    } else {
      uint256 _dividends = (value * _feeShareRatio) / RATE_BASE;
      uint256 _ownerIncome = value - _dividends;
  ...
  function _addDividend(uint256 amount) internal {
    ...
    uint256 _totalWeight = IPWS(_pwsAddress).totalWeightOf(_orderIds);
    for (uint256 i = 0; i < _orderIds.length; i++) {
      uint256 id = _orderIds[i];
      uint256 weight = IPWS(_pwsAddress).weight(id);
      uint256 revenue = (amount * weight) / _totalWeight;
